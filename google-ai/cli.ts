...
function parseArgs(): { query: string; engine: string; timeout: number; headless: boolean; help: boolean } {
    const result = {
        query: "",
        engine: "google",
        timeout: 30,
    ...
    for (let i = 0; i < args.length; i++) {
        ...
        if (arg === "--help" || arg === "-h") {
            ...
            // Unknown flag — skip it and its value if next arg exists and is not a flag
...
async function main() {
    ...
    try {
        ...
        console.error("\nBrowser left open for manual inspection. Close the window or press Ctrl+C to exit.");
        // Intentionally do not dispose — user wants to see the browser state
        return;
    } finally {
        if (success) {
